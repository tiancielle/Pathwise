...
import { RetourModal } from '../common/RetourModal'
...
export function ModuleCard({ module, index, onToggle, onClick }: ModuleCardProps) {
  ...
  const { state } = useApp()
  const { showModal, pendingTitre, handleOpenResource, handleConfirm, handleSkip } = useTrace()
